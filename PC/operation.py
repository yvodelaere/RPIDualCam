...
#%% RECTIFICATION
    ...
    while True: #Images are now available from the queue
        ...
        #%% Start of triggering
        if k==0:
            prevImg = np.zeros((fused.shape[0], fused.shape[1]), dtype=np.uint8)
        trigger, prevImg, idleCounter, captured, rad = checkTrigger(fused, prevImg, idleCounter, captured)
        if trigger:
